          ...
          {!selected ? <div className="grid min-h-[420px] place-items-center rounded-xl border border-dashed border-slate-800 bg-[#0b0f18] p-8 text-center text-sm text-slate-400">Create or select a replay session.</div> : <>
            <div className="rounded-xl border border-slate-800 bg-[#0b0f18] p-3">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-slate-100">{selected.symbol} · {selected.timeframe}m</h3><span className={`rounded border px-2 py-0.5 text-[10px] font-mono ${statusClass(selected.status)}`} data-testid="replay-status">{selected.status}</span>{stale && <span className="rounded border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[10px] text-amber-300">STALE STATE</span>}</div><div className="mt-1 truncate text-[10px] font-mono text-slate-500" data-testid="replay-selected-id">{selected.sessionId}</div><div className="mt-1 text-[10px] text-slate-500">Last processed candle: <span data-testid="replay-cursor">{formatTime(selected.cursorTime)}</span></div></div>
